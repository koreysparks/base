...
#define II(a, b, c, d, x, s, ac) { \
    ...
    const unsigned char* CMD5::digest() 
	{
        if (!m_finished) 
		{
            m_finished = true;
	...
	string CMD5::trim(const string &str)
	{
		if (str.empty())
		{
			return str;
	...
	{
		string c = trim(str);
        update((const unsigned char*)c.c_str(), c.length());
    }

    void CMD5::update(const unsigned char *input, size_t length)
	{
    ...
    void CMD5::encode(const unsigned int *input, unsigned char *output, size_t length) 
	{
    ...
    void CMD5::decode(const unsigned char *input, unsigned int *output, size_t length) 
	{
        for(size_t i=0, j=0; j<length; i++, j+=4)
		{	
    ...
    string CMD5::bytesToHexString(const unsigned char *input, size_t length)
	{
        string str;
